...
public class EntityManager {
    ...
    public void spawnEntity(Player target, Location location, String name, double health) {
        ...
        String bossName = PalutenBoss.getInstance().getBossName().replace("&6&l", "").replace("&e&l", "0");
        ...
        if (health > 2048 || health < 1) {
            ...
            if (PalutenBoss.getInstance().isDebugMode()) {
                Bukkit.getLogger().severe("health is lower or higher then the limit (reset to 750)");
            }
        }
        ...
        if (randomChance() % 6 == 0 && entityEquipment != null) {
            entityEquipment.setItemInHand(bossSword);
        }

        entity.setCustomName(entity.getCustomName() + " §8[§a" + entity.getHealth() + "§8]");
    }
            ...
            for (Entity entity : world.getEntities()) {
                if (entity instanceof Zombie zombie) {
                    if (zombie.isValid()) {
                        ...
                        if (customName != null && customName.contains(PalutenBoss.getInstance().getBossName())) {
                            zombie.remove();
                        }
                    }
                }
    ...
    public void scheduler() {
        new BukkitRunnable() {
            @Override
            public void run() {
                boolean showHealth = PalutenBoss.getInstance().getLoader().getConfigBuilder().getBoolean("entity.showHealth");
                for (World world : Bukkit.getWorlds()) {
                    for (Entity entity : world.getEntities()) {
                        if (entity.isValid() && entity instanceof Zombie zombie) {
                            String customName = entity.getCustomName();

                            if (customName != null && customName.contains(PalutenBoss.getInstance().getBossName())) {
                                if (showHealth) {
                                    String healthTitle = getString(zombie);
                                    entity.setCustomName(ChatColor.translateAlternateColorCodes('&', healthTitle));
                                } else {
                                    if (customName.contains("§7|")) {
                                        entity.setCustomName(PalutenBoss.getInstance().getBossName());
                                    }
                                }
                                ...
                                for (Player player : Bukkit.getOnlinePlayers()) {
                                    ...
                                    if (distance < 30) {
                                        player.sendActionBar(Component.text(entity.getCustomName()));
                                    }

                                }
                            }

                            double configHealth = PalutenBoss.getInstance().getLoader().getConfigBuilder().getDouble("entity.health");

                            if (zombie.getMaxHealth() != configHealth) {
                                zombie.setMaxHealth(configHealth);
                            }
            ...
            private @NotNull String getString(Zombie zombie) {
                String healthColor;

                double percentage = zombie.getHealth() / zombie.getMaxHealth();
                ...
                int healthValue = (int) Math.round(zombie.getHealth());

                return bossName + " §7| " + healthColor + healthValue;
            }

        }.runTaskTimer(PalutenBoss.getInstance(), 0, 20);
    }
}